...
window.PATCH_DATA = {
  ...
  skills: [
    ...
    { name: "Ancestral Warrior Totem", tag: "Warrior · Totem", kind: "buff", icon: "BruteAncestralWarriorTotem", chips: [{kind:"buff", text:"hidden 0.6s delay removed"}, {kind:"neutral", text:"now 50% of skill's attack time"}], notes: ['"Removed hidden 0.6s delay; now 50% of skill\'s attack time."'] },
    ...
    { name: "Feral Invocation", tag: "Druid · Spirit",      kind: "nerf",    icon: "4k/druidferalinvocation", chips: [{kind:"nerf", text:"no longer bypasses cooldown"}], notes: ['No longer bypasses cooldown.'] },
    ...
    { name: "Fragmentation Rounds", tag: "Mercenary · Crossbow", kind: "mixed", icon: "4k/burstshotphysical", chips: [{kind:"neutral", text:"Quality reworked"}, {kind:"nerf", text:"no more % Phys"}], notes: ['Quality grants +0–2 Fragments per Shot (was 0–20% more Physical Damage).'] },
    ...
    { name: "Ghost Dance",     tag: "Evasion · Buff",       kind: "mixed",   icon: "MonkGhostDance", chips: [{kind:"buff", text:"cooldown rate now applies"}, {kind:"buff", text:"2% Evasion/sec regen"}, {kind:"nerf", text:"shroud gain", from:"7.6–6.1s", to:"11.7–10.1s"}, {kind:"nerf", text:"loses shroud on hit"}], notes: ['Interval now uses Cooldown Recovery.', 'When Hit, lose a Ghost Shroud.', 'Regenerates 2% of your Evasion Rating per second.', 'Gains shroud 11.7–10.1s (was 7.6–6.1s).'] },
    ...
    { name: "Ice Shot",        tag: "Ranger · Cold",        kind: "nerf",    icon: "RangerIceShot", chips: [{kind:"nerf", text:"no more 25% more Freeze Buildup"}], notes: ['Ice Shards no longer have 25% more Freeze Buildup.'] },
    { name: "Ice Strike",      tag: "Monk · Combo",          kind: "buff",    icon: "MonkComboAttack", chips: [{kind:"buff", text:"Quality", from:"0–10% more AS", to:"0–20% increased AS"}], notes: ['Quality grants 0–20% increased Attack Speed (was 0–10% more).'] },
    ...
    { name: "Lunar Blessing",  tag: "Monk · Buff",          kind: "nerf",    icon: "4k/druidlunarblessing", chips: [{kind:"nerf", text:"Moonbeams no longer melee"}], notes: ['Triggered Moonbeams are no longer melee skills.'] },
    ...
    { name: "Rend",            tag: "Warrior · Bleed",      kind: "mixed",   icon: "4k/druidwyvernswipe", chips: [{kind:"neutral", text:"scaling bug fixed"}, {kind:"nerf", text:"Lightning-Charged dmg", from:"120–586%", to:"130–405%"}], notes: ['Fixed scaling bug.', 'Lightning-Charged deals 130–405% Attack Damage (was 120–586%).'] },
    ...
    { name: "Rolling Slam",    tag: "Warrior · Slam",       kind: "nerf",    icon: "BruteDoubleSlam", chips: [{kind:"nerf", text:"Attack time", from:"+1.5s", to:"+1.0s"}, {kind:"nerf", text:"1st slam", from:"90–367%", to:"75–272%"}, {kind:"nerf", text:"2nd slam", from:"180–735%", to:"150–543%"}], notes: ['Attack time +1s (was +1.5).', 'First slam 75–272% Attack Dmg (was 90–367%).', 'Second slam 150–543% (was 180–735%).'] },
    { name: "Shattering Spite", tag: "Sorceress",            kind: "nerf",    icon: "4k/atziriscontempt", chips: [{kind:"nerf", text:"50% less Magnitude of Ailments"}, {kind:"nerf", text:"instant leech removed"}], notes: ['50% Less Magnitude of Damaging Ailments.', 'Removed instant leech.'] },
    ...
    { name: "Shred",           tag: "Ranger · Bleed",       kind: "buff",    icon: "4k/druidfrozenclaws", chips: [{kind:"buff", text:"Quality", from:"0–10% more AS", to:"0–20% increased AS"}], notes: ['Quality grants 0–20% increased Attack Speed (was 0–10% more).'] },
    ...
    { name: "Tame Beast",      tag: "Druid · Beasts",       kind: "mixed",   icon: "HuntressTameBeast", chips: [{kind:"buff", text:"Beasts dmg", from:"baseline", to:"40–84% more (Lvl 9–20)"}, {kind:"buff", text:"summons immediately if spirit available"}], notes: ['Summoned beasts deal 40–84% more damage (levels 9–20).', 'Summons immediately if spirit available.'] },
    ...
    { name: "Time of Need",    tag: "Buff · Heal",          kind: "buff",    icon: "TimeOfNeedSkill", chips: [{kind:"buff", text:"Interval", from:"11.3–10.1s", to:"9.3–8.1s"}, {kind:"buff", text:"uses cooldown rate"}], notes: ['Uses Cooldown Recovery.', 'Interval 9.3–8.1s (was 11.3–10.1).'] },
    ...
    { name: "Whirling Assault", tag: "Warrior · Movement",  kind: "buff",    icon: "4k/monkwhirlingassault", chips: [{kind:"buff", text:"Quality", from:"0–15% more AS", to:"0–20% increased AS"}], notes: ['Quality grants 0–20% increased Attack Speed (was 0–15% more).'] },
  ...
  supports: [
    ...
    { name: "Advancing Storm",      kind: "buff",    icon: "s/advancingstorm", chips: [{kind:"buff", text:"now supports all Storm Skills"}], desc: 'Supports all Storm Skills under any conditions.' },
    ...
    { name: "Infernal Legion I",    kind: "buff",    icon: "s/infernallegion", chips: [{kind:"buff", text:"Life cost", from:"20%", to:"10%"}, {kind:"buff", text:"Minion ignite", from:"20%", to:"10%"}], desc: 'Takes 10% maximum Life as Fire (was 20%); ignites 10% of Minion Life (was 20%).' },
    { name: "Infernal Legion II",   kind: "buff",    icon: "s/infernallegion", chips: [{kind:"buff", text:"Life cost", from:"20%", to:"10%"}, {kind:"buff", text:"+20% Fire Res"}], desc: 'Takes 10% max Life (was 20%); ignites 10% Minion Life (was 20%); +20% Fire Resistance.' },
    { name: "Infernal Legion III",  kind: "nerf",    icon: "s/infernallegion", chips: [{kind:"nerf", text:"unobtainable"}], desc: 'Can no longer be obtained.' },
    ...
    { name: "Overextend",           kind: "nerf",    icon: "s/overextend", chips: [{kind:"nerf", text:"unobtainable"}], desc: 'Can no longer be obtained.' },
    ...
    { name: "Shock Conduction II",  kind: "nerf",    icon: "s/shockconduction", chips: [{kind:"nerf", text:"unobtainable"}], desc: 'Can no longer be obtained.' },
  ...
  passives: [
    ...
    { name: "Covering Ward", kind: "mixed", chips: [{kind:"buff", text:"ES on block", from:"20", to:"25"}, {kind:"buff", text:"+12% Block"}, {kind:"nerf", text:"no more 25% ES Recharge Rate"}], desc: 'Gain 25 ES on block (was 20), 12% increased Block Chance. Removed 25% ES Recharge Rate.' },
    ...
    { name: "Essence Infusion", kind: "mixed", chips: [{kind:"buff", text:"Int", from:"+10", to:"+12"}, {kind:"nerf", text:"no more 40% ES Rate"}], desc: '12% faster start of ES Recharge, +12 Int (was +10). Removed 40% ES Recharge Rate.' },
    ...
    { name: "Reformed Barrier cluster", kind: "nerf", chips: [{kind:"nerf", text:"cluster removed"}], desc: 'Removed.' },
  ...
  // ---------- UNIQUES — REWORKS ----------
  uniqueReworks: [
    { name: "Ab Aeterno", chips: [{kind:"buff", text:"AR/Ev/ES", from:"100–150%", to:"200–250%"}, {kind:"neutral", text:"reworked"}], desc: 'No longer 10% less Movement/Skill Speed per Dodge in past 20s. Now 200–250% AR/Ev/ES (was 100–150%), Speed unaffected by Slows while Sprinting, Gain Overencumbrance for 4s on Dodge Roll.' },
    { name: "Apep's Supremacy", chips: [{kind:"nerf", text:"lost 30–50% ES start"}], desc: 'No longer has 30–50% faster start of Energy Shield Recharge.' },
    { name: "Atziri's Acuity", chips: [{kind:"neutral", text:"reworked"}], desc: 'Reworked: 150–200% Armour, +100–150 Max Life, Leech 10% Phys Attack Dmg as Life, 10% Phys Hits cause Blood Loss & Vaal Pact, grants Herald of the Royal Queen.' },
    { name: "Sierran Inheritance", chips: [{kind:"neutral", text:"reworked"}], desc: 'Now 15–30% increased ES Recharge Rate (was 30–50% faster ES start).' },
    { name: "Blackflame", chips: [{kind:"buff", text:"Ignite Mag", from:"50%", to:"80–100%"}, {kind:"nerf", text:"Ignite Dur", from:"−50%", to:"−60 to −75%"}, {kind:"neutral", text:"reworked"}], desc: 'No longer "Ignites take Chaos instead of Fire". Now: Withers also increase Fire Dmg taken, Ignites deal Chaos instead of Fire, Withered doesn\'t expire on Ignited enemies. Ignite Magnitude 80–100% (was 50%), Ignite duration on enemies −60 to −75% (was −50%).' },
    { name: "Blistering Bond", chips: [{kind:"neutral", text:"reworked"}], desc: 'Fire Damage now contributes to Bleeding Magnitude; Bleeding deals Fire instead of Physical.' },
    { name: "Brass Dome", chips: [{kind:"nerf", text:"Armour", from:"700–800%", to:"500–600%"}], desc: '500–600% increased Armour (was 700–800%).' },
    { name: "Carnage Heart", chips: [{kind:"buff", text:"+25–50% Dmg while Leeching"}], desc: 'Now 25–50% increased Damage while Leeching.' },
    { name: "Chober Chaber", chips: [{kind:"buff", text:"+2–3 Minion Skill Levels"}], desc: 'Now +2–3 to Level of All Minion Skills.' },
    { name: "Death's Harp", chips: [{kind:"neutral", text:"reworked"}], desc: 'Lost "Bow Attacks Fire 3 Additional Arrows". Now +250–330% Surpassing Chance to Fire an Additional Arrow.' },
    { name: "Elevore", chips: [{kind:"buff", text:"Charm Charges/s", from:"0.5", to:"1"}, {kind:"buff", text:"+Charm Slots", from:"+1", to:"+1–2"}], desc: 'Charms gain 1 Charge per second (was 0.5); +1–2 Charm Slots (was +1).' },
    { name: "Eshtera's Path", chips: [{kind:"neutral", text:"reworked"}], desc: 'No longer "Cold Resistance unaffected by Area Penalties". Now: +5–10% Fire & Cold Res per Equipped Item with a Lightning Res Modifier.' },
    { name: "Glowswarm", chips: [{kind:"buff", text:"Mana Flask grants Guard"}], desc: 'Now: Using a Mana Flask grants Guard equal to 100% of the Flask\'s Recovery for 4 seconds.' },
    { name: "Hollow Mask", chips: [{kind:"buff", text:"grants Wildwood's Gifts"}, {kind:"buff", text:"Remnants affect allies"}], desc: 'Grants Wildwood\'s Gifts; Azmerian spirits show nearby Ancient Blooms. Remnants you create affect allies in Presence. 80–100% Reservation Efficiency of Remnant Skills.' },
    { name: "Husk of Dreams → Reverie", chips: [{kind:"neutral", text:"renamed & reworked"}], desc: 'Renamed Reverie. No longer 20–30% increased Flask Charges or 50% less Flask Charges used. Now: grants Rite of Restoration. Non-Unique Life Flasks apply effects constantly. Cannot use Life Flasks, no instant recovery, recovery only to self, 40–60% less Life Flask Recovery.' },
    { name: "Hyrri's Ire", chips: [{kind:"nerf", text:"Evasion", from:"200–250%", to:"100–150%"}, {kind:"nerf", text:"Cold Conv", from:"15–25%", to:"10–20%"}], desc: '100–150% increased Evasion (was 200–250%); Gain 10–20% Damage as Extra Cold (was 15–25%).' },
    { name: "Keeper of the Arc", chips: [{kind:"buff", text:"AR/ES", from:"150–250%", to:"240–340%"}], desc: '240–340% increased Armour and ES (was 150–250%).' },
    { name: "Levinstone", chips: [{kind:"neutral", text:"reworked"}], desc: 'No longer +1 to all Lightning Skills. Now: Lightning Skills Chain +1 times.' },
    { name: "Megalomaniac", chips: [{kind:"buff", text:"drops identified"}], desc: 'Now drops identified.' },
    { name: "Plaguefinger", chips: [{kind:"neutral", text:"reworked"}], desc: 'Now: Cannot inflict Elemental Ailments.' },
    { name: "Prized Pain", chips: [{kind:"neutral", text:"reworked"}], desc: 'No longer "deal Thorns to Stunned with Melee Attacks". Now 15–25% chance to deal Thorns to enemies Hit with Melee Attacks.' },
    { name: "Radiant Grief", chips: [{kind:"buff", text:"Ignite base", from:"100", to:"200"}], desc: 'Enemies in your Presence are Ignited as though dealt 200 Base Fire Damage (was 100).' },
    { name: "Safrin's Resolve", chips: [{kind:"neutral", text:"reworked"}], desc: 'No longer "Lightning Res unaffected by Area Penalties". Now: +5–10% Cold & Lightning Res per Equipped Item with a Fire Res Modifier.' },
    { name: "Seed of Cataclysm", chips: [{kind:"nerf", text:"lost 30–50% Crit Spell Dmg"}, {kind:"buff", text:"Lucky Spell Crit Dmg"}], desc: 'No longer 30–50% increased Crit Spell Dmg Bonus. Now: 5% reduced Crit Spell Dmg Bonus per recent Spell crit; 15–30% chance for Spell Crit Damage to be Lucky.' },
    { name: "Sekhema's Resolve (set)", chips: [{kind:"neutral", text:"renamed × 3"}], desc: 'Renamed to Safrin\'s Resolve, Zaida\'s Longevity, and Eshtera\'s Path.' },
    { name: "Sine Aequo", chips: [{kind:"buff", text:"AR/Ev/ES", from:"100–150%", to:"150–200%"}, {kind:"neutral", text:"reworked"}], desc: 'Now 150–200% AR/Ev/ES (was 100–150%). Lost "20% more Damage to Immobilised". Now 30–50% increased Damage against Immobilised.' },
    { name: "Soul Mantle", chips: [{kind:"neutral", text:"reworked"}], desc: 'No longer 20–30% reduced Totem Life. Now +75 to Spirit.' },
    { name: "Svalinn", chips: [{kind:"buff", text:"Cast on Block: 0 cost"}, {kind:"buff", text:"+50–100 Runic Ward"}], desc: 'Cast on Block: supported skills cost nothing. +50–100 Runic Ward (does not apply to existing items).' },
    { name: "Zaida's Longevity", chips: [{kind:"neutral", text:"reworked"}], desc: 'No longer "Lightning Res unaffected by Area Penalties". Now: +5–10% Fire & Lightning Res per Equipped Item with a Cold Res Modifier.' },
    { name: "Zerphi's Genesis", chips: [{kind:"neutral", text:"reworked"}], desc: 'No longer 10–30% increased Charm Charges used. Now: 50% Charges from used Life Flasks granted to Charms, 15–25% Cost Efficiency on Corrupted-Gem Skills while a Flask Effect is active, 25–50% increased Corrupted Charms Effect Duration.' },
  ],
